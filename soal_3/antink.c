...
#include <stdio.h>
#include <string.h>
#include <strings.h>    // untuk strncasecmp
#include <fcntl.h>
#include <dirent.h>
#include <errno.h>
#include <unistd.h>
...
// helper: cek apakah name (case-insensitive) mengandung keyword
...
static void *antink_init(struct fuse_conn_info *conn,
...
static int antink_readdir(const char *path, void *buf,
    ...
    while ((de = readdir(d))) {
        ...
        if (is_danger(de->d_name)) {
            // ALERT
            log_msg("[ALERT] Anomaly detected %s in file: %s",
                    de->d_name, dirp);
            // reverse name
            char rev[256];
            reverse_name(de->d_name, rev);
            // REVERSE
            log_msg("[REVERSE] File %s has been reversed : %s",
                    de->d_name, rev);
